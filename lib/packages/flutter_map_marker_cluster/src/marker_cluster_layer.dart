...
class _MarkerClusterLayerState extends State<MarkerClusterLayer>
  ...
  Future<void> _unspiderfy() async {
    switch (_spiderfyController.status) {
      case AnimationStatus.completed:
        List<Marker>? list =
            spiderfyCluster?.markers.map((MarkerNode markerNode) {
      ...
      case AnimationStatus.forward:
        final List<Marker>? markersGettingClustered = spiderfyCluster?.markers
            .map((MarkerNode markerNode) => markerNode.marker)
            .toList();
  ...
  VoidCallback _onClusterTap(MarkerClusterNode cluster) {
    return () async {
      ...
      final bool cannotDivide = cluster.markers.every((MarkerNode marker) =>
      ...
      final bool isAlreadyFit = latTween.begin == latTween.end &&
      ...
      final VoidCallback listener = _centerMarkerListener(
          animation, latTween, lonTween,
          zoomTween: zoomTween);
  ...
  VoidCallback _onMarkerTap(MarkerNode marker) {
    return () {
      ...
      final VoidCallback listener =
          _centerMarkerListener(animation, latTween, lonTween);
...
LatLngBounds _extendBounds(LatLngBounds bounds, double stickonFactor) {
  ...
  final LatLng point1 = LatLng((bounds.south - height).clamp(-90, 90),
      (bounds.west - width).clamp(-180, 180));
  final LatLng point2 = LatLng((bounds.north + height).clamp(-90, 90),
