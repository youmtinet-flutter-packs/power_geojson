...
import '../../marker_popup/src/options/popup_display_options.dart';
import '../../marker_popup/src/popup_layer.dart';
import '../../marker_popup/src/popup_spec.dart';
import '../../marker_popup/src/state/popup_state.dart';
...
class _MarkerClusterLayerState extends State<MarkerClusterLayer> with TickerProviderStateMixin {
  ...
  void _addLayers() {
    for (final Marker marker in widget.options.markers) {
  ...
  void _onMarkerHover(MarkerNode marker, bool enter) {
    ...
    if (widget.options.popupOptions != null) {
      final PopupOptions popupOptions = widget.options.popupOptions!;
      enter
          ? Future<void>.delayed(Duration(milliseconds: popupOptions.timeToShowPopupOnHover >= 0 ? popupOptions.timeToShowPopupOnHover : 0), () {
              if (!mounted) return;
  ...
  Future<void> _unspiderfy() async {
    switch (_spiderfyController.status) {
      case AnimationStatus.completed:
        List<Marker>? list = spiderfyCluster?.markers.map((MarkerNode markerNode) {
          return markerNode.marker;
        }).toList();
        final List<Marker>? markersGettingClustered = list;
      ...
      case AnimationStatus.forward:
        final List<Marker>? markersGettingClustered = spiderfyCluster?.markers.map((MarkerNode markerNode) => markerNode.marker).toList();
  ...
  void _addClusterClosingLayer(MarkerClusterNode clusterNode, List<Widget> layers) {
    ...
    // children
    final List<Marker> markersGettingClustered = <Marker>[];
    for (final MarkerOrClusterNode child in clusterNode.children) {
  ...
  List<Widget> _buildMobileTransformStack() {
    ...
    final int zoom = widget.mapCamera.zoom.ceil();
    final List<Widget> layers = <Widget>[];
    final List<Widget> spiderfyLayers = <Widget>[];
    ...
    // marker.
    final LatLngBounds recursionBounds = _extendBounds(
  ...
  VoidCallback _onClusterTap(MarkerClusterNode cluster) {
    return () async {
      ...
      // );
      final MarkerClusterLayerOptions opt = widget.options;
      ...
      final bool cannotDivide = cluster.markers.every((MarkerNode marker) => marker.parent!.zoom == _maxZoom && marker.parent == cluster.markers.first.parent) || (dest.zoom == _currentZoom && _currentZoom == opt.maxZoom);
      ...
      if (dest.zoom > _currentZoom && !cannotDivide) {
        _showPolygon(cluster.markers.map((MarkerNode m) => m.point).toList());
      }

      final Tween<double> latTween = Tween<double>(begin: center.latitude, end: dest.center.latitude);
      final Tween<double> lonTween = Tween<double>(begin: center.longitude, end: dest.center.longitude);
      final Tween<double> zoomTween = Tween<double>(begin: widget.mapCamera.zoom, end: dest.zoom);

      final bool isAlreadyFit = latTween.begin == latTween.end && lonTween.begin == lonTween.end && zoomTween.begin == zoomTween.end;
      ...
      final CurvedAnimation animation = CurvedAnimation(parent: _fitBoundController, curve: widget.options.animationsOptions.fitBoundCurves);

      final VoidCallback listener = _centerMarkerListener(animation, latTween, lonTween, zoomTween: zoomTween);
  ...
  VoidCallback _onMarkerTap(MarkerNode marker) {
    return () {
      ...
      final LatLng center = widget.mapCamera.center;
      final Tween<double> latTween = Tween<double>(begin: center.latitude, end: marker.point.latitude);
      final Tween<double> lonTween = Tween<double>(begin: center.longitude, end: marker.point.longitude);
      ...
      final VoidCallback listener = _centerMarkerListener(animation, latTween, lonTween);
  ...
  void _showPolygon(List<LatLng> points) {
    if (widget.options.showPolygon) {
      setState(() {
        _polygon = PolygonLayer<Object>(polygons: <Polygon<Object>>[
          Polygon<Object>(
  ...
  List<Point<double>?> _generatePointSpiderfy(int count, Point<double> center) {
...
LatLngBounds _extendBounds(LatLngBounds bounds, double stickonFactor) {
  ...
  final double height = (sw.latitude - ne.latitude).abs() * stickonFactor;
  final double width = (sw.longitude - ne.longitude).abs() * stickonFactor;
  ...
  final LatLng point1 = LatLng((bounds.south - height).clamp(-90, 90), (bounds.west - width).clamp(-180, 180));
  final LatLng point2 = LatLng((bounds.north + height).clamp(-90, 90), (bounds.east + width).clamp(-180, 180));
