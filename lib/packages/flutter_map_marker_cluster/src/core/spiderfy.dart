...
class Spiderfy {
  ...
  static List<Point<double>?> spiral(int distanceMultiplier, int count, Point<double> center) {
    ...
    final List<Point<double>?> result = List<Point<double>?>.filled(count, null, growable: false);
    ...
    for (int i = count; i >= 0; i--) {
      ...
      if (i < count) {
        result[i] = Point<double>(center.x + legLength * cos(angle), center.y + legLength * sin(angle));
      }
  ...
  static List<Point<double>?> circle(int radius, int count, Point<double> center) {
    ...
    return List<Point<double>>.generate(count, (int index) {
      ...
      return Point<double>(center.x + radius * cos(angle), center.y + radius * sin(angle));
    });
  }
}
