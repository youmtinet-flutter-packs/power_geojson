...
import '../popup_container/popup_container_mixin.dart';
import '../popup_snap.dart';
import '../popup_spec.dart';
import '../state/popup_event.dart';
import '../state/popup_state_impl.dart';
...
class _AnimatedPopupContainerState extends State<AnimatedPopupContainer> with PopupContainerMixin {
  ...
  void showPopupsAlsoFor(ShowedPopupsAlsoForEvent event) {
    for (final PopupSpec popupSpec in event.popupSpecs) {
  ...
  void showPopupsOnlyFor(ShowedPopupsOnlyForEvent event) {
    _animatedStackManager.removeWhere(
      (PopupSpec popupSpec) => !event.popupSpecs.contains(popupSpec),
      duration: event.disableAnimation ? Duration.zero : null,
    );
    for (final PopupSpec popupSpec in event.popupSpecs) {
      if (!_animatedStackManager.contains(popupSpec)) {
